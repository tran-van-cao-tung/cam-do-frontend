...
const AddContract = ({ setShowAddContract }) => {
  ...
  const hanldeSubmit = (e) => {
    ...
    console.log(data);
    axios.post(`http://tranvancaotung-001-site1.atempurl.com/api/v1/contract/createContract`, data, { headers: { "Authorization": `Bearer ${localStorage.getItem("accessToken")}` } }).then(res => {
  ...
  useEffect(() => {
    axios.get('http://tranvancaotung-001-site1.atempurl.com/api/v1/package/getAll/0', { headers: { "Authorization": `Bearer ${localStorage.getItem("accessToken")}` } }).then(res => {
      setPackage(res.data);
    })
  }, [])
  ...
  const hanldePackageItem = (e) => {
    setPackageItem(packagelist.filter((item) => {
  ...
  const handleCustomer = (e) => {
    ...
    axios.get(`http://tranvancaotung-001-site1.atempurl.com/api/v1/customer/getByCCCD/${value}`, { headers: { "Authorization": `Bearer ${localStorage.getItem("accessToken")}` } }).then(res => {
  ...
  const handleSeri = (e) => {
    axios.get(`http://tranvancaotung-001-site1.atempurl.com/api/v1/pawnableProduct/getPawnAbleProductById/${e.target.value}`, { headers: { "Authorization": `Bearer ${localStorage.getItem("accessToken")}` } }).then(res => {
                    ...
                    <Grid item xs={12} md={6}>
                      <div className="user__info">
                        ...
                        <div className="user__info-input">
                          <input type="text" placeholder="Nhập tên khách hàng" name="customerName" onChange={e => handleInput(e)} value={customer ? customer.fullName : contract.customerName} />
                          <input type="text" name="cccd" placeholder="Nhập CMND/Hộ chiếu" onMouseOut={(e) => handleCustomer(e)} />
                        </div>
                      </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <div className="user__info">
                        ...
                        <div className="user__info-input">
                          <input type="text" name="phone" placeholder="Nhập số điện thoại" value={customer ? customer.phone : ""} />
                          <input
                            type="text"
                            name="address" placeholder="Nhập địa chỉ khách hàng"
                      ...
                      <div className="user__info">
                        ...
                        <div className="user__info-input">
                          ...
                          <select onChange={e => handleSeri(e)}>
                            <option>---Loại tài sản---</option>
                            {
                              pawnableProduct.map((item, index) => {
                                return <option value={item.pawnableProductId} key={index} >{item.typeOfProduct}</option>
                              })
                            }
                          </select>
                          ...
                          <div className="box__input">
                            <input type="number" name="insuranceFee"
                              onChange={(e) => { handleSum(e) }} placeholder="0" />
                            <span>VNĐ</span>
                          </div>
                          <div className="box__input">
                            <input type="number" name="storageFee" onChange={(e) => { handleSum(e) }} placeholder="0" />
                            <span>VNĐ</span>
                          </div>
                          <div className="box__input">
                          ...
                          <span style={{ width: "100%", height: "10px" }}>{user ? user.fullName : ""}</span>
                        </div>
                      </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <div className="user__info">
                        ...
                        <div className="user__info-input">
                          ...
                          <select onChange={(e) => hanldePackageItem(e)} >
                            <option>---Gói Cầm---</option>
                            {
                              packagelist.map((item, index) => {
                                return <option key={index} value={item.packageId} >{item.packageName}</option>
                              })
                            }
                          </select>
                          ...
                          {/* <input type="date" /> */}
                          <p className="flcenter">{packageItem[0] ? packageItem[0].packageInterest : ""}%</p>
                          <input type="number" name='interestRecommend' onChange={e => handleInput(e)} />
                          <p className="flend">{ }</p>
                        </div>
                      </div>
                    </Grid>
                  ...
                  <Grid container spacing={2}>
                      ...
                      <div className="user__info">
                        ...
                        <div className="user__info-input">
                          {/* <input type="text" name="name" placeholder="Nhập tên khách hàng" value={seri[0] ? seri[0].attributes.length : ""} /> */}
                          {
                            attributes ? (attributes.map((item, index) => {
                              return (
                                <input type="text" name={index} onChange={e => hanleInputAttribute(e, item.pawnableProductId, index)} placeholder={`Nhập ${item.description}`} />
                              )
                            })) : ""
                          }
                    ...
                    <Grid item xs={12} md={6}>
                      <div className="btn__group">
                        ...
                        <button className="btn btn__close" onClick={() => setShowAddContract(false)}>
                          <img src={close} alt="" />
                          Đóng
                        </button>
                      </div>
