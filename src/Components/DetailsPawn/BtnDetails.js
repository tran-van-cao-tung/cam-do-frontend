...
const BtnDetails = ({ rowsContract, setContract }) => {
  ...
  const handleSubmit = () => {
    ...
    console.log(data);
    setContract(rowsContract.filter((item) => {
      if (
        data.customerName !== "" &&
        item.customerName
          .toLowerCase()
          .includes(data.customerName.toLowerCase())
      ) {
        return true;
      }
      if (
        data.contractStartDate !== "" &&
        formatDate(item.contractStartDate) === data.contractStartDate
      ) {
        return true;
      }
      if (
        data.contractEndDate !== "" &&
        formatDate(item.contractEndDate) === data.contractEndDate
      ) {
        return true;
      }
      if (
        data.commodityCode !== "" &&
        item.commodityCode === data.commodityCode
      ) {
        return true;
      }
      if (
    ...
    <div className="btn-detail">
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={2}>
            <select className="lts" onChange={e => handleOptionLTS(e)}>
          ...
          <Grid item xs={2}>
            <select className="lts" onChange={e => handleOptionStatus(e)}>
